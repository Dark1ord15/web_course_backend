...
func (a *Application) StartServer() {
	...
	RoadGroup := r.Group("/roads")
	{
		...
		RoadGroup.POST("/road_travel_request/:id", a.WithAuthCheck(role.Buyer), roadController.AddRoadToTravelRequest)
	...
	TravelRequestGroup := r.Group("/travelrequests")
	{
		...
		TravelRequestGroup.GET("/introduced", a.WithAuthCheck(role.Buyer), travelRequestController.GetTravelRequestByID)
		TravelRequestGroup.PUT("/:id", a.WithAuthCheck(role.Buyer), travelRequestController.UpdateTravelRequest)
		TravelRequestGroup.PUT("/change-status-user/:id", a.WithAuthCheck(role.Buyer), travelRequestController.ChangeRequestStatusByUser)
		TravelRequestGroup.DELETE("/:id", a.WithAuthCheck(role.Buyer), travelRequestController.SoftDeleteTravelRequest)
		TravelRequestGroup.POST("/pay/:id", a.WithAuthCheck(role.Buyer), travelRequestController.PayTravelRequest)
	...
	{
		TravelRequestRoadGroup.DELETE("/:requestID/:roadID", a.WithAuthCheck(role.Buyer), travelRequestRoadController.PhysicalDeleteRoadFromTravelRequest)
	}
